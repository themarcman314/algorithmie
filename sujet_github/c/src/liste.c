...
int main(void)
{
	...
	struct Link *my_node = create_node(123);
	printf("List 1\n");
	print_list(ma_liste);
	printf("\n");
...
struct Link *create_link(struct Node *initial, struct Node *destination)
{
...
void add_value(struct List *list, struct Node *initial, struct Node *destination)
{
	struct Link *current = list->root;
	if(current == NULL)
...
struct List copy_list(struct List *l)
{
	...
	struct Link *current_node = l->root;
